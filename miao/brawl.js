		...
		brawl:{
			...
			qianlidanji:{
				...
				showcase:function(init){
					...
					var player1;
					if(init){
						player1=ui.create.player(null,true).init('caiyang');
						player1.node.marks.remove();
						player1.node.hp.remove();
						player1.style.left='20px';
						player1.style.top='20px';
					...
					if(!_status.qianlidanji) _status.qianlidanji={
						...
						replace_character:function(){
							...
							if(_status.qianlidanji.completeNumber!=5){
 							var list=_status.qianlidanji.completeReward.randomGets(4);
							list[0]='不做选择';
 							var list2=[null];
							...
							_status.event.getParent(3).num = 0
				...
				content:{
					...
					chooseCharacterBefore:function(){
   					...
   					event.list.randomSort();
   					_status.characterlist=event.list.slice(0);
   					var list=event.list.slice(0,20);
   					...
   					game.me.chooseButton(dialog,true,[1,Infinity]).set('onfree',true);
   					ui.create.cheat=function(){
   						_status.createControl=ui.cheat2;
   						ui.cheat=ui.create.control('更换',function(){
   							if(ui.cheat2&&ui.cheat2.dialog==_status.event.dialog){
					...
					if(result.buttons.length >= 2){
						game.zhu.init(result.buttons[0].link,result.buttons[1].link);
						for(var i = 0;i < result.buttons.length;++i){
							game.addRecentCharacter(result.buttons[i].link);
							_status.characterlist.remove(result.buttons[i].link);
							_status.qianlidanji.used.add(result.buttons[i].link);
						}
						for(var i = 2;i < result.buttons.length;++i){
							target = result.buttons[i].link;
							var list=[];
